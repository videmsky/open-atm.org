  ...
  m.Details = function(options) {
    ...
    function _refreshDetail(id) {
        ...
        $.ajax({
            ...
            crossDomain: true,
            dataType: 'jsonp',
            success: function (mobileobject, textStatus, jqXHR) {            
                // Structure the data a bit
                setImages(mobileobject);

                // Set the page title
                $(_options.detailHeader, $container).html(mobileobject.title);
                
                var detailsHtml = imageHtml = '';
                detailsHtml += '<div class="details_title">'+mobileobject.title+'</div>';

                if(mobileobject.imgs.length > 0) {
                    detailsHtml += (mobileobject.imgs[0] != "images/noimage.png") ? '<img src="'+mobileobject.imgs[0]+'" />' : '';
                    if(mobileobject.imgs.length > 1) {
                        for(var i=1; i < mobileobject.imgs.length; i++) {
                            imageHtml += '<img src="'+mobileobject.imgs[i]+'" />';
                        }
                    }
                }
                detailsHtml += '<ul>';
                // Dump everything else onto the page
                detailsHtml += '<li><strong>Description</strong> '+mobileobject.description+'</li>';
                $.each(mobileobject, function(i, n) {
